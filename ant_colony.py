...
class AntColony:
    ...
    def heuristic(self, node, final_node):
        ''' Heuristic function to estimate the distance to the final node '''
        return np.sqrt((node[0] - final_node[0])**2 + (node[1] - final_node[1])**2)

    def select_next_node(self, actual_node):
        ''' Select the next node to visit based on pheromone levels and heuristic information '''
        total_sum = sum(edge['Pheromone'] / (1.0 + self.heuristic(edge['FinalNode'], self.map.final_node)) for edge in actual_node.edges)
        for edge in actual_node.edges:
            edge['Probability'] = (edge['Pheromone'] / (1.0 + self.heuristic(edge['FinalNode'], self.map.final_node))) / total_sum
